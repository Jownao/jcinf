  ...
  if (variant === "animated") {
    return (
      <motion.a
        ...
        whileTap={{ scale: 0.97 }}
        className={`relative inline-flex overflow-hidden rounded-full p-[2px] transition-colors ${className}`}
      >
        <span className="absolute inset-[-1000%] animate-spin-slow bg-[conic-gradient(from_90deg_at_50%_50%,#EFF6FF_0%,#2563EB_50%,#EFF6FF_100%)]" />
        <span className={`inline-flex h-full w-full items-center justify-center gap-2 rounded-full bg-[#2563EB] text-white hover:bg-[#1d4ed8] transition-colors shadow-md shadow-blue-200 backdrop-blur-3xl ${sizeStyles[size]}`}>
          {content}
        </span>
      </motion.a>
    );
